...
description: "Configure which trade actions (enter, invert, tighten) are allowed at training time with SignalPolicy. Runtime policies can only narrow — never widen — the trained policy."
---
...
interface SignalPolicy {
  allow: SignalAction[];  // "enter" | "invert" | "tighten"
...
<ResponseField name="allow" type="SignalAction[]" required>
...
`minRiskReward` and `rrMetric` are purely runtime — they are not baked into the model at training time, because the RR statistics (`model.riskReward.bySymbol`) are already part of the trained params. The runtime policy can only **tighten** an existing `minRiskReward` (raise the threshold), never lower it.
